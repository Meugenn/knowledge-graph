...
export class RALPHEngine {
  ...
  async _loop() {
    while (
      ...
      try {
        ...
        console.error('RALPH iteration error (non-fatal, continuing):', err);
        // Don't let a single iteration crash the entire loop
      }
    ...
    if (
      ...
      if (lastReport) {
        try {
          ...
          const newPapers = await this._fetchAndScore(queries, lastReport, this.guidance, lastIter.papers || lastIter.fullPapers);
          ...
          console.warn('RALPH queue replenishment failed:', err.message);
        }
      }
    }
  ...
  async _runIteration() {
    ...
    let newPapers = [];
    try {
      newPapers = await this._fetchAndScore(queries, report, this.guidance, batch);
    } catch (err) {
      console.warn('RALPH paper fetch failed (non-fatal):', err.message);
    }
    this.queue.push(...newPapers);
    this.callbacks.onQueueChange?.([...this.queue]);
